...
function DocumentCard({ icon, title, details, color }) {
  return (
    <div className="group relative bg-gradient-to-br from-white via-white to-blue-50/20 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border border-blue-200/50 hover:border-blue-300 transform hover:-translate-y-1">
      ...
      <div className={`relative z-10 bg-gradient-to-br ${color} p-4 overflow-hidden`}>
        {/* Pattern overlay */}
      ...
      <div className="p-6 sm:p-8 space-y-3">
        {checklist.map((item, index) => (
          ...
          >
            ...
            <p className="text-slate-700 text-sm sm:text-base leading-snug flex-1">{item}</p>
          </div>
        ))}
      </div>
    </div>
      ...
      {[
        ...
        >
          <div className="flex items-center space-x-3 mb-3">
            <div className={`p-3 rounded-xl shadow-md bg-gradient-to-br ${card.gradient} flex items-center justify-center`}>
              {card.icon}
            </div>
            <h4 className="text-base sm:text-lg font-bold text-slate-900">{card.title}</h4>
          </div>
          <p className="text-slate-700 text-sm sm:text-base leading-snug">{card.description}</p>
          <div className="mt-4 flex items-center text-blue-600 font-medium text-sm cursor-pointer hover:underline">
          </div>
        </div>
      ))}
    </div>
          ...
          <div key={index} className="flex items-start gap-3">
            <div className="w-6 h-6 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
              <svg className="w-3.5 h-3.5 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1.414 0z" clipRule="evenodd" />
              </svg>
            </div>
