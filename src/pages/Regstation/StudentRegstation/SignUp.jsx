...
import { FaUser, FaEnvelope, FaLock, FaPhone, FaGraduationCap, FaGlobe, FaBookOpen, FaArrowRight, FaEye, FaEyeSlash } from "react-icons/fa";
...
import { validateEmail, validatePassword, validateConfirmPassword, validateRequired, getPasswordStrength } from "../../../utils/validation";
...
const SignUp = () => {
  ...
  const [passwordStrength, setPasswordStrength] = useState({ level: 0, color: "gray", text: "" });
  ...
  const handleCountryCodeChange = (e) => {
    ...
    if (newFormData.mobile && code) {
      ...
      } catch (err) { newPhoneError = "Invalid phone number"; }
    }
    setFormData(newFormData);
    setPhoneError(newPhoneError);
    setPhoneValid(newPhoneValid);
  ...
  const handleNationalityChange = (e) => {
    ...
    if (matchedCountry) {
      ...
      if (!matchingPhone) {
        ...
        matchingPhone = phonecode.find(p => p.name && p.name.toLowerCase() === cName);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (phoneError) { toast.error(phoneError); setLoading(false); return; }
    if (!phoneValid && formData.mobile) { toast.error("Please enter a valid phone number"); setLoading(false); return; }
    ...
    try {
      ...
      const studentId = response.data?.id || response.data?.data?.id || response.data?.student_id;
  ...
  const findCountryForPhoneCode = (phoneCodeValue) => {
    ...
    let iso = phoneCodeData?.iso || phoneCodeData?.country_code || phoneCodeData?.sortname;
    if (!iso && phoneCodeData?.name) {
      const matchedCountry = countriesData.find(c => c.name === phoneCodeData.name);
      iso = matchedCountry?.iso || matchedCountry?.sortname || matchedCountry?.code;
    }
    return iso || 'US';
  };
  ...
  return (
    <div className="min-h-screen w-full flex bg-white font-sans">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-blue-600 sticky top-0 h-screen overflow-hidden flex-col justify-between p-16 text-white text-left">
        ...
        <div className="relative z-10 animate-fade-in-right">
          ...
          <h1 className="text-5xl font-extrabold leading-tight mb-6 tracking-tight">
            Start Your <br /> <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-200 to-white">Study in Malaysia</span> <br /> Journey Today!
          </h1>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 lg:p-12 relative bg-white overflow-y-auto min-h-screen">
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-3">
                <div className="relative w-1/3">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 text-lg pointer-events-none z-10">{formData.country_code && getCountryFlag(findCountryForPhoneCode(formData.country_code))}</div>
                  <select
                    name="country_code"
            ...
            <div className="space-y-1.5 p-4 bg-gray-50 rounded-xl border border-gray-100">
              ...
              <div className="flex items-center gap-3">
                ...
                <button type="button" onClick={generateCaptcha} className="p-2.5 text-gray-500 hover:text-blue-600 hover:bg-white rounded-lg transition-all border border-transparent hover:border-gray-200 hover:shadow-sm">
                  <LuRefreshCw />
                </button>
                <input
                  type="text"
            ...
            <p className="text-center text-sm text-gray-500">
              ...
              <Link to="/login" className="font-bold text-blue-600 hover:text-blue-700 hover:underline ml-1">Sign In</Link>
            </p>
          </form>
        </div>
      </div>
...
// Modern Select
const ModernSelect = ({ label, icon, options, name, value, onChange, onBlur, required, error }) => (
  <div className="space-y-1.5">
    <label className="text-sm font-semibold text-gray-700 ml-1">{label}</label>
    <div className="relative group">
      <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-blue-600 z-10 transition-colors">{icon}</div>
      <select
        name={name}
        value={value}
        onChange={onChange}
        onBlur={onBlur}
        required={required}
        className={`appearance-none w-full pl-11 pr-10 py-3.5 bg-gray-50 border rounded-xl text-gray-900 font-medium focus:bg-white focus:ring-4 transition-all outline-none text-sm cursor-pointer ${error
          ? "border-red-300 focus:border-red-500 focus:ring-red-100"
          : "border-gray-200 focus:border-blue-500 focus:ring-blue-500/10"
          }`}
      >
        <option value="" disabled hidden className="text-gray-500">Select {label}</option>
        {options.map((opt, idx) => (
...
const PasswordInput = ({ label, name, placeholder, value, onChange, onBlur, showPassword, setShowPassword, icon, required, error, showStrength, strength }) => (
  <div className="space-y-1.5">
    <label className="text-sm font-semibold text-gray-700 ml-1">{label}</label>
    <div className="relative group">
      <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-blue-600 transition-colors">{icon}</div>
    ...
    {showStrength && value && strength && strength.level > 0 && (
      <div className="flex items-center gap-2 mt-1">
        ...
        <span className={`text-xs font-medium ${strength.color === 'red' ? 'text-red-600' :
