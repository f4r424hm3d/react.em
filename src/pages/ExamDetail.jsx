...
const formatExamDescription = (html) => {
    ...
    return html
        ...
        .replace(/<h2>(.*?)<\/h2>/g, `<h2 class="text-xl sm:text-2xl md:text-3xl font-extrabold text-blue-900 border-l-4 border-blue-600 pl-3 sm:pl-4 mb-2 mt-4">${'$1'}</h2>`)
        .replace(/<h3>(.*?)<\/h3>/g, `<h3 class="text-lg sm:text-xl md:text-2xl font-bold text-gray-800 border-b-2 border-gray-200 pb-2 mb-2 mt-3">${'$1'}</h3>`)
        .replace(/<h4>(.*?)<\/h4>/g, `<h4 class="text-base sm:text-lg md:text-xl font-semibold text-gray-700 mb-1 mt-2">${'$1'}</h4>`)

        // Tables - mobile responsive
        .replace(/<table/g, `<div class="overflow-x-auto -mx-4 sm:mx-0 my-2 rounded-lg"><table class="w-full text-xs sm:text-sm md:text-base border-collapse"`)
        .replace(/<\/table>/g, `</table></div>`) // Close the wrapper div
        .replace(/<thead>/g, `<thead class="bg-blue-600 text-white text-left">`)
        .replace(/<th>/g, `<th scope="col" class="px-2 sm:px-4 md:px-6 py-2 sm:py-3 font-semibold text-xs sm:text-sm md:text-base">`)
        ...
        .replace(/<td>/g, `<td class="px-2 sm:px-4 md:px-6 py-2 sm:py-3 text-gray-800 text-xs sm:text-sm md:text-base border-b border-gray-200">`);
};
const Sidebar = () => (
    <div className="space-y-4 sm:space-y-6 lg:sticky lg:top-20">
        <div className="bg-white rounded-xl sm:rounded-2xl shadow p-4 sm:p-6">
            <h2 className="text-lg sm:text-xl font-bold text-gray-800 border-b pb-3 sm:pb-4 mb-4 sm:mb-5">Important Exams</h2>
            {['PTE', 'TOEFL', 'MUET'].map((exam) => (
                ...
                >
                    <span className="font-medium text-sm sm:text-base group-hover:translate-x-1 transition">{exam}</span>
                ...
                <div className="max-w-screen-xl mx-auto px-3 sm:px-4 py-2 sm:py-3">
                    <div className="flex items-center flex-wrap gap-1 sm:gap-2 text-xs sm:text-sm text-gray-600">
                        <Link to="/" className="flex items-center gap-1 hover:underline hover:text-blue-500">
                            <Home size={14} className="sm:w-[18px] sm:h-[18px]" /> <span className="hidden sm:inline">Home</span>
                        </Link>
                        <span>/</span>
                        <span className="flex items-center gap-1">
                            <Layers size={14} className="sm:w-[18px] sm:h-[18px] hidden sm:inline" />Resources
                        </span>
                        <span>/</span>
                        ...
                        <span>/</span>
                        <span className="flex items-center gap-1 text-blue-600 font-medium truncate max-w-[100px] sm:max-w-none">
                            {slug?.toUpperCase()}
                        </span>
                    </div>
                </div>
            ...
            <div className="min-h-screen bg-gray-50 py-4 sm:py-8 md:py-12 px-3 sm:px-4 md:px-6 lg:px-8">
                <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-10">
                    <div className="lg:col-span-2 bg-white p-4 sm:p-6 md:p-8 rounded-xl shadow-lg">
                        <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold text-gray-900 mb-3 sm:mb-4 md:mb-6 leading-tight">{exam.page_name} - Exam Details</h1>
                        ...
                        <p className="text-gray-700 mb-4 sm:mb-6 md:mb-8 text-sm sm:text-base md:text-lg lg:text-xl font-medium leading-relaxed">{exam.headline}</p>
